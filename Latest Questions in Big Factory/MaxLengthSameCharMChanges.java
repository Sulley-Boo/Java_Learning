...
public class MaxLengthSameCharMChanges {
    ...
    // 滑动窗口
    public static int maxLength1(String s, int[] arr, int m) {
        char[] chs = s.toCharArray();
        int n = s.length();
        int ans = 0;
        for (char c = 'a'; c <= 'z'; c++) { // 尝试讲字符修改为c
            int r = 0;
            int change = 0;
            for (int l = 0; l < n; l++) {
                while (r < n) {
    ...
    // 暴力方法
    public static int maxLength2(String str, int[] arr, int m) {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < testTimes; i++) {
            ...
            int ans1 = maxLength1(str, arr, m);
            int ans2 = maxLength2(str, arr, m);
            if (ans1 != ans2) {
